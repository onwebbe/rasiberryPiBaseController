...
class RasiberryPiGPIO:
  ...
  def getPin(self, number):
    ...
    for pin in self.pins:
      ...
      if (pinID == number):
        return pin
    return Pin.Pin(-1, self.mode, '', -1, -1, 'IN') # return an un functionable pin object to prevent None exception
  ...
  def _getPinByPITypeAuto(self):
    ...
    for pinData in dataList:
      ...
      mode = pinJson['mode']
      pinObj = Pin.Pin(pin, self.mode, name, bcm, pin, mode)
  ...
  def _getPinByPIType(self):
    type = self.type
    if (type == '3B+'):
      self.pins.append(Pin.Pin(1, self.mode, '3.3V', -1, 1, 'IN'))
      self.pins.append(Pin.Pin(2, self.mode, '5V', -1, 2, 'IN'))
      self.pins.append(Pin.Pin(3, self.mode, 'SDA.1', 2, 3, 'IN'))
      self.pins.append(Pin.Pin(4, self.mode, '5V', -1, 4, 'IN'))
      self.pins.append(Pin.Pin(5, self.mode, 'SCL.1', 3, 5, 'IN'))
      self.pins.append(Pin.Pin(6, self.mode, 'GND', -1, 6, 'IN'))
      self.pins.append(Pin.Pin(7, self.mode, 'GPIO.7', 4, 7, 'IN'))
      self.pins.append(Pin.Pin(8, self.mode, 'TXD', 14, 8, 'IN'))
      self.pins.append(Pin.Pin(9, self.mode, 'GND', -1, 9, 'IN'))
      self.pins.append(Pin.Pin(10, self.mode, 'RXD', 15, 10, 'IN'))
      self.pins.append(Pin.Pin(11, self.mode, 'GPIO.0', 17, 11, 'IN'))
      self.pins.append(Pin.Pin(12, self.mode, 'GPIO.1', 18, 12, 'IN'))
      self.pins.append(Pin.Pin(13, self.mode, 'GPIO.2', 27, 13, 'IN'))
      self.pins.append(Pin.Pin(14, self.mode, 'GND', -1, 14, 'IN'))
      self.pins.append(Pin.Pin(15, self.mode, 'GPIO.3', 22, 15, 'IN'))
      self.pins.append(Pin.Pin(16, self.mode, 'GPIO.4', 23, 16, 'IN'))
      self.pins.append(Pin.Pin(17, self.mode, '3.3C', -1, 17, 'IN'))
      self.pins.append(Pin.Pin(18, self.mode, 'GPIO.5', 24, 18, 'IN'))
      self.pins.append(Pin.Pin(19, self.mode, 'MOSI', 10, 19, 'IN'))
      self.pins.append(Pin.Pin(20, self.mode, 'GND', -1, 20, 'IN'))
      self.pins.append(Pin.Pin(21, self.mode, 'MISO', 9, 21, 'IN'))
      self.pins.append(Pin.Pin(22, self.mode, 'GPIO.6', 25, 22, 'IN'))
      self.pins.append(Pin.Pin(23, self.mode, 'SCLK', 11, 23, 'IN'))
      self.pins.append(Pin.Pin(24, self.mode, 'CE0', 8, 24, 'IN'))
      self.pins.append(Pin.Pin(25, self.mode, 'GND', 0, 25, 'IN'))
      self.pins.append(Pin.Pin(26, self.mode, 'CE1', 7, 26, 'IN'))
      self.pins.append(Pin.Pin(27, self.mode, 'SDA.0', 0, 27, 'IN'))
      self.pins.append(Pin.Pin(28, self.mode, 'SCL.0', 1, 28, 'IN'))
      self.pins.append(Pin.Pin(29, self.mode, 'GPIO.21', 5, 29, 'IN'))
      self.pins.append(Pin.Pin(30, self.mode, 'GND', -1, 30, 'IN'))
      self.pins.append(Pin.Pin(31, self.mode, 'GPIO.22', 6, 31, 'IN'))
      self.pins.append(Pin.Pin(32, self.mode, 'GPIO.26', 12, 32, 'IN'))
      self.pins.append(Pin.Pin(33, self.mode, 'GPIO.23', 13, 33, 'IN'))
      self.pins.append(Pin.Pin(34, self.mode, 'GND', -1, 34, 'IN'))
      self.pins.append(Pin.Pin(35, self.mode, 'GPIO.24', 19, 35, 'IN'))
      self.pins.append(Pin.Pin(36, self.mode, 'GPIO.27', 16, 36, 'IN'))
      self.pins.append(Pin.Pin(37, self.mode, 'GPIO.25', 26, 37, 'IN'))
      self.pins.append(Pin.Pin(38, self.mode, 'GPIO.28', 20, 38, 'IN'))
      self.pins.append(Pin.Pin(39, self.mode, 'GND', -1, 39, 'IN'))
      self.pins.append(Pin.Pin(40, self.mode, 'GPIO.29', 21, 40, 'IN'))
